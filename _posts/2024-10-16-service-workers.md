...
**服务工作线程**（*[service workers](https://developer.mozilla.org/en-US/docs/Web/API/Service_Worker_API/Using_Service_Workers)*）也是工作线程的一种，它的作用就像代理服务器，可以拦截 web 应用发出的请求，并根据网络情况，要么返回真实的服务器响应，要么返回本地缓存。
...
注意，存入本地缓存的响应是通过 [`clone()`](https://developer.mozilla.org/en-US/docs/Web/API/Response/clone) 克隆函数产生的副本。这一步很重要，因为响应流（*response streams*）只能被读取一次。通过克隆产生的副本存入本地，原始响应返回请求发起方，双方都能拿到可读取的数据，皆大欢喜。
